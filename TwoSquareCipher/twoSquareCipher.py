...
#key = "example"
key = input(print("Enter a key: "))

def generate_table(key):
    alphabet = 'ABCDEFGHIKLMNOPQRSTUVWXYZ_'
...
def call_cipher():
    input_file_name = "new_text.txt"
    output_file_name = "new_ciphered_text.txt"
    ...
    file_input.close()
    ciphered = encrypt(key, content)
    file_output = open(output_file_name, 'w')
    file_output.write(ciphered)
    file_output.close()

def call_decipher():
    input_file_name = "new_ciphered_text.txt"
    if not os.path.exists(input_file_name):
